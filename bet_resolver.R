...
get_correct_player_stat <- function(stats_data, requested_stat, sport = "mlb") {
  ...
  if (sport != "mlb" && sport != "baseball" && sport != "soccer") {
  ...
  if (sport == "soccer") {
    soccer_mapping <- list(
      goals = c("goals", "goal"),
      assists = c("assists", "assist"),
      shots = c("shots", "shot"),
      shots_on_target = c("shots_on_target", "sot"),
      cards_yellow = c("cards_yellow", "yellow_cards", "yellow_card"),
      cards_red = c("cards_red", "red_cards", "red_card"),
      fouls_committed = c("fouls_committed", "fouls"),
      fouls_drawn = c("fouls_drawn")
    )
    possible_fields <- soccer_mapping[[requested_stat]]
    if (is.null(possible_fields)) possible_fields <- c(requested_stat)
...
map_team_name <- function(team_name) {
  ...
  if (team_name %in% names(team_map)) {
...
normalize_sport_name <- function(sport) {
  ...
  # SOCCER
  else if (sport_lower %in% c("soccer", "football", "futbol", "european football")) {
    return("soccer")
  }
  else {
    return(sport_lower)
...
# ==============================================
# CONSISTENT BET WIN/LOSS LOGIC
# ==============================================
check_bet_won <- function(actual_value, line_value, market_string, player_name = NULL, direction = NULL) {
  tryCatch({
    if (is.null(actual_value) || is.na(actual_value)) {
      return(list(won = NULL, line = NULL))
    }

    market_lower <- tolower(market_string)
    player_lower <- if (!is.null(player_name)) tolower(player_name) else ""
    direction_lower <- if (!is.null(direction)) tolower(direction) else ""
    combined_lower <- paste(market_lower, player_lower, direction_lower)

    # Try to get line from line_value first
    line_val_num <- suppressWarnings(as.numeric(line_value))

    # If line_value is NA or NULL, try to extract from market/player strings
    if (is.null(line_val_num) || is.na(line_val_num)) {
      # Extract number that looks like a line (e.g., "Over 4.5" or "24.5")
      # Look for direction + space + number
      match_dir <- regmatches(combined_lower, regexec("(over|under)\\s+([0-9.]+)", combined_lower))[[1]]
      if (length(match_dir) >= 3) {
        line_val_num <- as.numeric(match_dir[3])
      } else {
        # Try just extracting a number at the end
        match_end <- regmatches(combined_lower, regexpr("[0-9.]+$", combined_lower))
        if (length(match_end) > 0) {
          line_val_num <- as.numeric(match_end[1])
        }
      }
    }

    if (is.na(line_val_num)) {
      # One last check: maybe it's just a number string?
      if (!is.null(line_value) && !is.na(line_value) && grepl("^[0-9.]+$", as.character(line_value))) {
        line_val_num <- as.numeric(as.character(line_value))
      }
    }
    ...
    # Determine direction
    is_over <- grepl("over", combined_lower)
    is_under <- grepl("under", combined_lower)

    if (!is_over && !is_under) {
      return(list(won = NULL, line = line_val_num))
    }

    # Result logic
    # Prioritize over if both found
    won <- if (is_over) actual_value > line_val_num else actual_value < line_val_num

    debug_cat(sprintf("  check_bet_won: actual=%s, line=%s, direction=%s, won=%s\n",
                      actual_value, line_val_num, ifelse(is_over, "over", "under"), won))

    return(list(won = won, line = line_val_num))

  }, error = function(e) {
    debug_cat(sprintf("  ERROR in check_bet_won: %s\n", e$message))
    return(list(won = NULL, line = NULL))
  })
}
...
resolve_game_market <- function(event_string, sport, season, market_type, line_value = NULL, direction = NULL, team = NULL, game_date = NULL) {
  ...
  # Handle 1st Inning Moneyline (3-way)
  if (grepl("1st inning moneyline", market_type, ignore.case = TRUE) || 
      grepl("first inning moneyline", market_type, ignore.case = TRUE)) {
    
    debug_cat(sprintf("  Handling 1st inning moneyline for %s\n", sport))
  ...
  # Handle 1st 5 Innings Moneyline
  if (grepl("1st 5 innings moneyline", market_type, ignore.case = TRUE) || 
      grepl("first 5 innings moneyline", market_type, ignore.case = TRUE)) {
    
    debug_cat(sprintf("  Handling first 5 innings moneyline for %s\n", sport))
  ...
  # =========================================================
  # HANDLE HALF MARKETS (First Half = 5 innings in MLB)
  # =========================================================
  
  # Handle "1st Half Moneyline" (5 innings in MLB)
  if (grepl("1st half moneyline", market_type, ignore.case = TRUE) || 
      grepl("first half moneyline", market_type, ignore.case = TRUE)) {
    
    debug_cat(sprintf("  Handling first half (5 innings) moneyline for %s\n", sport))
    ...
    is_3way <- grepl("3-way", market_type, ignore.case = TRUE) || 
      grepl("3 way", market_type, ignore.case = TRUE)
    
    inning_result <- resolve_multi_inning_market(
      game_id = game_id,
      num_innings = 5,  # First half = first 5 innings
      market_type = if(is_3way) "moneyline_3way" else "moneyline",
      bet_team = if(is_3way) bet_side else NULL
    )
  ...
  # Handle "1st Half Total Runs" (5 innings)
  if (grepl("1st half total runs", market_type, ignore.case = TRUE) || 
      grepl("first half total runs", market_type, ignore.case = TRUE)) {
    
    debug_cat(sprintf("  Handling first half (5 innings) total runs for %s\n", sport))
    
    line_num <- as.numeric(line_value)
    if (is.na(line_num) && !is.null(line_value)) {
      line_num <- suppressWarnings(as.numeric(gsub("[^0-9.-]", "", line_value)))
    }
  ...
  # Handle "2nd Half Moneyline" (innings 6-9)
  if (grepl("2nd half moneyline", market_type, ignore.case = TRUE) || 
      grepl("second half moneyline", market_type, ignore.case = TRUE)) {
    
    debug_cat(sprintf("  Handling second half (innings 6-9) moneyline for %s\n", sport))
    
    # Get full game scores and subtract first 5 innings
    full_game <- extract_cumulative_scores(game_id, "mlb")
    first_5 <- extract_cumulative_scores(game_id, "mlb", innings = 5)
    
    home_2nd_half <- full_game$home_score - first_5$home_score
    away_2nd_half <- full_game$away_score - first_5$away_score
    ...
    if (home_2nd_half > away_2nd_half) {
      winner <- "home"
      bet_won <- (bet_side == "home")
    } else if (away_2nd_half > home_2nd_half) {
      winner <- "away"
      bet_won <- (bet_side == "away")
    } else {
      winner <- "tie"
      bet_won <- if(is_3way) (bet_side == "tie") else NA
    }
    ...
    return(list(
      ...
      away_score = away_2nd_half,
      game_id = game_id
    ))
  }
  
  # =========================================================
  # GENERIC INNING/PERIOD MARKET HANDLERS
  # =========================================================
  
  # Handle ANY "X Inning/Y Period Moneyline" pattern
  if (grepl("\\d+(st|nd|rd|th)?\\s+(inning|period|quarter|half|round)", market_type, ignore.case = TRUE) && 
      grepl("moneyline", market_type, ignore.case = TRUE)) {
    
    # Skip if already handled by specific half handlers
    if (!grepl("half", market_type, ignore.case = TRUE)) {
      inning_num <- as.numeric(gsub(".*?(\\d+).*", "\\1", market_type))
      if (is.na(inning_num) || inning_num == 0) inning_num <- 1
      
      debug_cat(sprintf("  Handling generic %d inning/period moneyline for %s\n", inning_num, sport))
      ...
      market_type_3way <- if(is_3way) "moneyline_3way" else "moneyline"
      
      inning_result <- resolve_multi_inning_market(
        game_id = game_id,
        num_innings = inning_num,
        market_type = market_type_3way,
        bet_team = if(is_3way) bet_side else NULL
      )
      
      suffix <- if(inning_num == 1) "st" else if(inning_num == 2) "nd" else if(inning_num == 3) "rd" else "th"
  ...
  # Handle ANY "X Inning/Y Period Total Runs/Points" pattern
  if (grepl("\\d+(st|nd|rd|th)?\\s+(inning|period|quarter|half|round)", market_type, ignore.case = TRUE) && 
      (grepl("total runs", market_type, ignore.case = TRUE) || 
       grepl("total points", market_type, ignore.case = TRUE))) {
    ...
    if (!grepl("half", market_type, ignore.case = TRUE)) {
      ...
      debug_cat(sprintf("  Handling generic %d inning/period total for %s (line: %s, direction: %s)\n", 
                        inning_num, sport, line_num, direction))
  ...
  # Handle "Both Teams To Score" in ANY period
  if (grepl("both teams to score", market_type, ignore.case = TRUE)) {
    
    if (grepl("\\d+(st|nd|rd|th)?\\s+(inning|period|quarter|half)", market_type, ignore.case = TRUE)) {
      inning_num <- as.numeric(gsub(".*?(\\d+).*", "\\1", market_type))
    ...
    debug_cat(sprintf("  Handling Both Teams To Score for %s\n", 
                      ifelse(is.null(inning_num), "full game", paste0("inning ", inning_num))))
  ...
  if (grepl("team\\s+\\d+(st|nd|rd|th)?\\s+(inning|period|quarter)?\\s+total runs", market_type, ignore.case = TRUE)) {
    
    inning_num <- as.numeric(gsub(".*?(\\d+).*", "\\1", market_type))
    if (is.na(inning_num)) inning_num <- 1
    
    debug_cat(sprintf("  Handling team inning total runs for inning %d\n", inning_num))
    
    result <- extract_cumulative_scores(game_id, "mlb", innings = inning_num)
    
    if (grepl(tolower(teams$home), tolower(team), ignore.case = TRUE)) {
      actual_score <- result$home_score
    } else {
      actual_score <- result$away_score
    }
    ...
    bet_won <- if (tolower(direction) == "over") actual_score > line_num else actual_score < line_num
  ...
  if (sport %in% c("nba", "basketball")) {
    ...
    return(resolve_mlb_game(game_id, market_type, line_value, direction, team, teams))
  } else if (sport %in% c("nhl", "hockey")) {
    return(resolve_nhl_game(game_id, market_type, line_value, direction, team, teams))
  } else if (sport %in% c("nfl", "football")) {
    return(resolve_nfl_game(game_id, season, market_type, line_value, direction, team, teams))
  } else if (sport %in% c("ncaaf", "ncaab")) {
    return(resolve_college_game(game_id, sport, market_type, line_value, direction, team, teams))
  } else if (sport == "soccer") {
    return(resolve_soccer_game(game_id, market_type, line_value, direction, team, teams, event_string))
  } else {
    return(list(
      success = FALSE,
      resolved = FALSE,
  ...
  if (grepl("moneyline", market_lower)) {
      ...
      if (!is.null(bet_team)) {
        ...
        if (bet_team == "home") {
          ...
          if (is.na(adjusted_score) || is.na(result$away_score)) {
            result$won <- FALSE
          } else if (adjusted_score == result$away_score) {
            result$won <- NA # PUSH
          } else {
          ...
          if (is.na(adjusted_score) || is.na(result$home_score)) {
            result$won <- FALSE
          } else if (adjusted_score == result$home_score) {
            result$won <- NA # PUSH
          } else {
    ...
    if (!is.null(line_value) && !is.null(direction)) {
      ...
      if (is.na(result$total_points)) {
        result$won <- FALSE
      } else if (result$total_points == line_num) {
        result$won <- NA # PUSH
      } else {
...
resolve_mlb_game <- function(game_id, market_type, line_value, direction, team, teams) {
  ...
  data <- tryCatch({
    ...
    debug_cat(sprintf("Error fetching ESPN MLB data: %s\n", e$message))
    return(NULL)
  })
  ...
  if (is.null(data)) {
    return(list(
      success = FALSE,
      resolved = FALSE,
      error = "Could not fetch MLB game data from ESPN",
      game_id = game_id
    ))
  }
  ...
  # Check status
  status <- data$header$competitions$status$type$description %||% "Unknown"
  if (!grepl("Final|Completed", status, ignore.case = TRUE)) {
    return(list(
      success = TRUE,
      resolved = FALSE,
      message = paste("Game not yet completed:", status),
      is_future = TRUE
    ))
  }
  ...
  if (!is.null(team) && nchar(team) > 0) {
    ...
    if (grepl(team_lower, home_lower) || grepl(home_lower, team_lower) || team_lower == "home") {
      bet_team <- "home"
    } else if (grepl(team_lower, away_lower) || grepl(away_lower, team_lower) || team_lower == "away") {
      bet_team <- "away"
    }
  }
  ...
  if (grepl("moneyline", market_lower)) {
    ...
    result$bet_type <- "moneyline"
  } else if (grepl("spread|run.*line", market_lower)) {
    if (!is.null(line_value)) {
      line_num <- as.numeric(line_value)
      if (!is.null(bet_team)) {
        if (bet_team == "home") {
          adjusted_score <- result$home_score + line_num
          if (adjusted_score == result$away_score) {
            result$won <- NA # PUSH
          } else {
            result$won <- adjusted_score > result$away_score
          }
          ...
          adjusted_score <- result$away_score + line_num
          if (adjusted_score == result$home_score) {
            result$won <- NA # PUSH
          } else {
            result$won <- adjusted_score > result$home_score
          }
    ...
    if (!is.null(line_value) && !is.null(direction)) {
      line_num <- as.numeric(line_value)
      result$actual_value <- result$total_points
      if (result$total_points == line_num) {
        result$won <- NA # PUSH
      } else if (tolower(direction) == "over") {
        result$won <- result$total_points > line_num
      } else {
        result$won <- result$total_points < line_num
      }
...
resolve_nhl_game <- function(game_id, market_type, line_value, direction, team, teams) {
  ...
  data <- tryCatch({
    ...
    debug_cat(sprintf("Error fetching ESPN NHL data: %s\n", e$message))
    return(NULL)
  })
  ...
  if (is.null(data)) {
    return(list(
      success = FALSE,
      resolved = FALSE,
      error = "Could not fetch NHL game data from ESPN",
      game_id = game_id
    ))
  }
  ...
  if (grepl("moneyline", market_lower)) {
    ...
    result$bet_type <- "moneyline"
  } else if (grepl("spread|puck.*line", market_lower)) {
    if (!is.null(line_value)) {
      line_num <- as.numeric(line_value)
      if (!is.null(bet_team)) {
        if (bet_team == "home") {
...
resolve_college_game <- function(game_id, sport, market_type, line_value, direction, team, teams) {

  debug_cat(sprintf("\n=== RESOLVING COLLEGE %s GAME ===\n", toupper(sport)))

  sport_path <- if (sport == "ncaaf") "football/college-football" else "basketball/mens-college-basketball"

  # Try ESPN API
  url <- paste0("https://site.api.espn.com/apis/site/v2/sports/", sport_path, "/summary?event=", game_id)
  data <- tryCatch({
    jsonlite::fromJSON(url)
  }, error = function(e) {
    debug_cat(sprintf("Error fetching ESPN College data: %s\n", e$message))
    return(NULL)
  })
  ...
  if (is.null(data)) {
    return(list(
      success = FALSE,
      resolved = FALSE,
      error = paste("Could not fetch College game data from ESPN for", sport),
      game_id = game_id
    ))
  }
  ...
  if (grepl("moneyline", market_lower)) {
    ...
    result$bet_type <- "moneyline"
  } else if (grepl("spread", market_lower)) {
    if (!is.null(line_value)) {
      line_num <- as.numeric(line_value)
      if (!is.null(bet_team)) {
        if (bet_team == "home") {
...
resolve_soccer_game <- function(game_id, market_type, line_value, direction, team, teams, event_string = "") {

  debug_cat("\n=== RESOLVING SOCCER GAME ===\n")

  # Soccer usually uses league-specific endpoints.
  # Use helper to get slug from event_string (or default to eng.1)
  league_slug <- get_espn_league_slug(event_string)
  debug_cat(sprintf("Using league slug: %s (detected from '%s')\n", league_slug, event_string))

  url <- paste0("https://site.api.espn.com/apis/site/v2/sports/soccer/", league_slug, "/summary?event=", game_id)
  data <- tryCatch({
    jsonlite::fromJSON(url)
  }, error = function(e) {
    debug_cat(sprintf("Error fetching ESPN data: %s\n", e$message))
  ...
  if (is.null(data)) {
    return(list(
      success = FALSE,
      resolved = FALSE,
      error = "Could not fetch soccer game data from ESPN",
      game_id = game_id
    ))
  }
  ...
  # Check status
  status <- data$header$competitions$status$type$description %||% "Unknown"
  if (!grepl("Full Time|Final", status, ignore.case = TRUE)) {
    return(list(
      success = TRUE,
      resolved = FALSE,
      message = paste("Game not yet completed:", status),
  ...
  if (grepl("moneyline|result", market_lower)) {
    # 3-way resolution (Home/Draw/Away)
    if (bet_on == "draw" || bet_on == "tie") {
      result$won <- result$home_score == result$away_score
    } else {
      # Identify if bet is on home or away team
      home_name <- tolower(result$home_team)
      away_name <- tolower(result$away_team)

      if (grepl(bet_on, home_name) || grepl(home_name, bet_on) || bet_on == "home") {
        result$won <- result$home_score > result$away_score
      } else if (grepl(bet_on, away_name) || grepl(away_name, bet_on) || bet_on == "away") {
        result$won <- result$away_score > result$home_score
      } else {
        # Default fallback: check which team it matched
        result$won <- FALSE
        result$error <- paste("Could not match bet team:", team)
      }
    }
    result$bet_type <- "moneyline"
  } else if (grepl("double chance", market_lower)) {
    # Double Chance resolution (Home or Draw, Away or Draw, Home or Away)
    home_name <- tolower(result$home_team)
    away_name <- tolower(result$away_team)

    if (grepl("home|draw", bet_on) && grepl("draw", bet_on)) {
      # Home or Draw
      result$won <- result$home_score >= result$away_score
    } else if (grepl("away|draw", bet_on) && grepl("draw", bet_on)) {
      # Away or Draw
      result$won <- result$away_score >= result$home_score
    } else if (grepl("home", bet_on) && grepl("away", bet_on)) {
      # Home or Away
      result$won <- result$home_score != result$away_score
    } else {
      # Fallback for specific team names
      if (grepl(home_name, bet_on)) {
        result$won <- result$home_score >= result$away_score
      } else if (grepl(away_name, bet_on)) {
        result$won <- result$away_score >= result$home_score
      } else {
        result$won <- FALSE
        result$error <- paste("Could not resolve double chance for:", team)
      }
    }
    result$bet_type <- "double_chance"
  } else if (grepl("both teams to score|btts", market_lower)) {
    # BTTS resolution
    both_scored <- result$home_score > 0 && result$away_score > 0
    if (grepl("yes", bet_on)) {
      result$won <- both_scored
    } else if (grepl("no", bet_on)) {
      result$won <- !both_scored
    } else {
      result$won <- FALSE
      result$error <- paste("Could not resolve BTTS for:", team)
    }
    result$bet_type <- "btts"
  } else if (grepl("draw no bet", market_lower)) {
    # Draw No Bet resolution
    if (result$home_score == result$away_score) {
      result$won <- NA # PUSH/VOID
    } else {
      home_name <- tolower(result$home_team)
      away_name <- tolower(result$away_team)
      ...
      if (grepl(bet_on, home_name) || grepl(home_name, bet_on) || bet_on == "home") {
        ...
        result$won <- FALSE
        result$error <- paste("Could not match bet team for DNB:", team)
      }
    }
    result$bet_type <- "draw_no_bet"
  } else if (grepl("spread|handicap", market_lower)) {
    # Spread/Handicap resolution (including Asian Handicap)
    line_num <- as.numeric(line_value)
    home_name <- tolower(result$home_team)
    away_name <- tolower(result$away_team)

    # Determine if bet is on home or away
    is_home <- grepl(bet_on, home_name) || grepl(home_name, bet_on) || bet_on == "home"
    is_away <- grepl(bet_on, away_name) || grepl(away_name, bet_on) || bet_on == "away"

    if (is_home) {
      # Adjust home score by handicap and compare to away score
      diff <- result$home_score + line_num - result$away_score
    } else if (is_away) {
      # Adjust away score by handicap and compare to home score
      diff <- result$away_score + line_num - result$home_score
    } else {
      result$won <- FALSE
      result$error <- paste("Could not match team for handicap:", team)
      return(result)
    }

    # Handle half-win/half-loss for Asian Handicaps (e.g. +/- 0.25, 0.75, 1.25)
    # For now, we'll treat them as full win if > 0, push if 0, loss if < 0
    # Note: 0.25 lines will never result in diff == 0
    if (diff > 0) {
      result$won <- TRUE
    } else if (diff == 0) {
      result$won <- NA # PUSH
...
resolve_special_prop <- function(game_id, player_name, sport, season, special_type, line_value = NULL) {
  ...
  tryCatch({
    if (sport %in% c("nba", "basketball", "wnba")) {
      # Use your existing ESPN function
      league_slug <- if (sport == "wnba") "wnba" else "nba"
      stats <- get_espn_nba_player_stats(game_id, player_name, league = league_slug)
...
classify_market <- function(market_type, sport = NULL) {
  ...
  if (!is.null(sport) && tolower(sport) == "wnba") {
    # Total points for WNBA (game total)
    if (grepl("total points", market_lower) || market_lower == "total_points") {
      return(list(type = "game_total"))
    }
    # Team total points for WNBA (e.g., "Indiana Fever Total Points")
    if (grepl("team total points", market_lower) || grepl("total points", market_lower) && nchar(market_lower) > 0) {
      # This will be handled by the player_name parameter in resolve_bet
      return(list(type = "game_total"))
    }
    # Player points for WNBA
    if (grepl("player points", market_lower)) {
      return(list(type = "player_stat", stat = "points"))
    }
    # Player rebounds for WNBA
    if (grepl("player rebounds", market_lower)) {
      return(list(type = "player_stat", stat = "rebounds"))
    }
    # Player assists for WNBA
    if (grepl("player assists", market_lower)) {
      return(list(type = "player_stat", stat = "assists"))
    }
  }
  ...
  player_stats <- c(
    ...
    "shots on target", "yellow cards", "red cards", "fouls committed", "fouls drawn"
  )
  ...
  if (grepl("total points", market_lower) || grepl("total runs", market_lower) || grepl("total goals", market_lower)) {
    ...
    # WNBA, NBA, NCAA basketball all use total points
    return(list(type = "game_total"))
  }
...
# ==============================================
# ==============================================
# PERIOD TOTALS RESOLVER - CORRECTED VERSION
# ==============================================
...
resolve_period_total <- function(game_id, sport, period, line_value = NULL, direction = NULL) {
  debug_cat <- function(...) { cat(..., file = stderr()) }
  debug_cat(sprintf("\nResolving %s total for %s (direction: %s)\n", period, sport, ifelse(is.null(direction), "none", direction)))
  tryCatch({
    if (sport %in% c("nba", "basketball")) {
      url <- paste0("https://site.api.espn.com/apis/site/v2/sports/basketball/nba/summary?event=", game_id)
      ...
      if (status_code(response) == 200) {
        ...
        if (!is.null(data$header$competitions$competitors)) {
          comps <- data$header$competitions$competitors[[1]]
          home_scores <- c(); away_scores <- c()
          for (i in 1:nrow(comps)) {
            if (!is.null(comps$linescores[[i]])) {
              linescores <- comps$linescores[[i]]
              scores <- as.numeric(linescores$displayValue)
              if (comps$homeAway[i] == "home") { home_scores <- scores } else { away_scores <- scores }
            }
          }
          if (length(home_scores) > 0 && length(away_scores) > 0) {
            total <- 0
            if (grepl("1st", period)) total <- away_scores[1] + home_scores[1]
            else if (grepl("2nd", period)) total <- away_scores[2] + home_scores[2]
            else if (grepl("3rd", period)) total <- away_scores[3] + home_scores[3]
            else if (grepl("4th", period)) total <- away_scores[4] + home_scores[4]
            if (total > 0) {
              result <- list(success = TRUE, actual_value = total, total = total, period = period)
              if (!is.null(direction) && !is.null(line_value)) {
                line_num <- as.numeric(line_value)
                if (direction == "over") result$bet_won <- total > line_num
                else if (direction == "under") result$bet_won <- total < line_num
                result$line_value <- line_num
              }
              return(result)
            }
  ...
  }, error = function(e) { return(list(success = FALSE, error = paste("Error:", e$message))) })
}
...
detect_correct_season <- function(sport, game_date, current_year = NULL) {
  ...
  if (sport %in% c("nfl", "football")) {
    ...
    # Soccer season usually runs Aug-May (spans calendar years)
    # Example: Game on Dec 14, 2025 -> Season 2025
    # Example: Game on Jan 14, 2026 -> Season 2025
    ...
    debug_cat(sprintf("  Soccer: Game %s-%s -> Season %s (current year: %s)\n",
                      game_year, game_month, season, current_year))
...
find_nba_game_id_timezone_simple <- function(game_date, away_team_search, home_team_search) {
  ...
  clean_team_name <- function(name) { name <- tolower(trimws(name)); name <- gsub("[^a-z0-9]", "", name); name }
  home_search_clean <- clean_team_name(home_team_search)
  away_search_clean <- clean_team_name(away_team_search)
  ...
  for (offset in date_offsets) {
    ...
    if (length(games) > 0) {
      ...
      for (game in games) {
        game_home_clean <- clean_team_name(game$home_team)
        game_away_clean <- clean_team_name(game$away_team)
        if (game_away_clean == away_search_clean && game_home_clean == home_search_clean) {
          debug_cat(sprintf("      ✓ EXACT MATCH!\n"))
          return(list(game_id = game$game_id, actual_date = as.character(current_date), searched_date = as.character(base_date), days_off = offset, match_type = "exact", note = "Found exact team order match"))
        }
      }
      for (game in games) {
        game_home_clean <- clean_team_name(game$home_team)
        ...
        if (game_away_clean == home_search_clean && game_home_clean == away_search_clean) {
          debug_cat(sprintf("      ⚠️ SWAPPED MATCH (teams reversed)\n"))
          return(list(game_id = game$game_id, actual_date = as.character(current_date), searched_date = as.character(base_date), days_off = offset, match_type = "swapped", note = "Teams found but home/away reversed", correct_home = game$away_team, correct_away = game$home_team))
        }
      }
    }
  }
...
# ==============================================
# FIXED: Get NBA games on a specific date with error handling
# ==============================================
...
get_all_nba_games_on_date <- function(game_date) {
  tryCatch({
    ...
    debug_cat(sprintf("    API URL: %s\n", url))
    
    response <- GET(url, timeout = 10)
    
    # Handle non-200 responses gracefully
    if (status_code(response) != 200) {
      debug_cat(sprintf("    API returned status %d, skipping\n", status_code(response)))
      return(list())
    }
    ...
    data <- tryCatch({
      ...
      debug_cat(sprintf("    JSON parse error: %s\n", e$message))
      return(NULL)
    })
    
    if (is.null(data) || is.null(data$events) || nrow(data$events) == 0) {
      debug_cat(sprintf("    No events found on %s\n", game_date))
      return(list())
    }
      ...
      if (!is.null(game$competitions) && length(game$competitions) > 0) {
        ...
        if (!is.null(comp$competitors) && length(comp$competitors) > 0) {
          ...
          for (j in 1:nrow(competitors)) {
            ...
            if ("homeAway" %in% colnames(competitor)) {
              # Extract team name - handle different possible structures
              team_name <- ""
              if ("team.displayName" %in% colnames(competitor)) {
                team_name <- competitor$team.displayName
              } else if (is.list(competitor$team) && "displayName" %in% names(competitor$team)) {
                team_name <- competitor$team$displayName
              } else if ("team.name" %in% colnames(competitor)) {
              ...
              if (!is.na(competitor$homeAway) && competitor$homeAway == "home") {
                ...
                if ("score" %in% colnames(competitor)) {
                  home_score <- as.numeric(competitor$score)
                }
              } else if (!is.na(competitor$homeAway) && competitor$homeAway == "away") {
                away_team <- team_name
                if ("score" %in% colnames(competitor)) {
                  away_score <- as.numeric(competitor$score)
                }
              }
            }
          }
          ...
          if (home_team != "Unknown" && away_team != "Unknown") {
            # Get game status
            game_status <- "Unknown"
            if ("status.type.description" %in% colnames(game)) {
              game_status <- game$status.type.description
            } else if (!is.null(comp$status$type$description)) {
            ...
            games_list[[length(games_list) + 1]] <- list(
              ...
              score = paste(ifelse(is.na(away_score), "?", away_score), "-", 
                            ifelse(is.na(home_score), "?", home_score)),
              status = game_status
            )
          }
        }
    ...
    debug_cat(sprintf("    Found %d games on %s\n", length(games_list), game_date))
    return(games_list)
    
  }, error = function(e) {
    debug_cat(sprintf("    Error in get_all_nba_games_on_date: %s\n", e$message))
    return(list())
  })
}

# Also update find_nba_game_id_timezone_simple to use this properly
find_nba_game_id_timezone_simple <- function(game_date, away_team_search, home_team_search) {
  debug_cat(sprintf("🕐 Timezone-aware search for %s @ %s on %s\n",
                    away_team_search, home_team_search, game_date))
  ...
  # Try dates in order: exact, -1, +1, -2, +2
  date_offsets <- c(0, -1, 1, -2, 2, -3, 3)
  ...
  for (offset in date_offsets) {
    ...
    if (length(games) > 0) {
      ...
      # FIRST: Look for exact team match (away/home order must match)
      for (game in games) {
        game_home_clean <- clean_team_name(game$home_team)
        game_away_clean <- clean_team_name(game$away_team)
        
        debug_cat(sprintf("      Checking: %s @ %s (ID: %s)\n", game$away_team, game$home_team, game$game_id))
      ...
      for (game in games) {
        ...
        if (game_away_clean == home_search_clean && game_home_clean == away_search_clean) {
          ...
          return(list(
            ...
            days_off = offset,
            match_type = "swapped",
            note = "Teams found but home/away reversed - adjust spread calculation",
            correct_home = game$away_team,
            correct_away = game$home_team
          ))
        }
      }
    } else {
      debug_cat(sprintf("    No games found\n"))
    }
  }
...
# ==============================================
# SOCCER GAME LOOKUP (ESPN API)
# ==============================================

get_espn_soccer_games <- function(game_date, league_slug = "eng.1") {
  date_str <- format(as.Date(game_date), "%Y%m%d")
  url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/soccer/%s/scoreboard?dates=%s", league_slug, date_str)
  debug_cat(sprintf("  Fetching soccer games from: %s\n", url))

  res <- tryCatch({
    jsonlite::fromJSON(url)
  }, error = function(e) {
    debug_cat(sprintf("  Error fetching soccer scoreboard: %s\n", e$message))
    return(NULL)
  })

  if (is.null(res) || is.null(res$events) || length(res$events) == 0) {
    return(data.frame())
  }
...
find_soccer_game_id <- function(home_team, away_team, game_date, league = NULL) {
  debug_cat(sprintf("\nSearching for Soccer game: %s vs %s on %s\n", away_team, home_team, game_date))

  # Common leagues to check if none provided
  ...
  for (league_slug in leagues_to_check) {
    debug_cat(sprintf("  Checking league: %s\n", league_slug))

    # Try exact date
    game_id <- find_soccer_game_id_single_date(home_team, away_team, game_date, league_slug)
    if (!is.na(game_id)) return(game_id)

    # Try ±1 day
    for (offset in c(-1, 1)) {
      check_date <- as.Date(game_date) + offset
      game_id <- find_soccer_game_id_single_date(home_team, away_team, check_date, league_slug)
      if (!is.na(game_id)) {
        debug_cat(sprintf("  ✓ Found on %s! Game ID: %s\n", check_date, game_id))
        return(game_id)
      }
    }
  }
...
find_soccer_game_id_single_date <- function(home_team, away_team, game_date, league_slug) {
  tryCatch({
    games <- get_espn_soccer_games(game_date, league_slug)

    if (nrow(games) == 0) return(NA)
    ...
    for (i in 1:nrow(games)) {
      game_name <- tolower(games$name[i])
      game_clean <- clean_name(game_name)

      # Check if both teams are in the game name
      if (grepl(home_clean, game_clean) && grepl(away_clean, game_clean)) {
        debug_cat(sprintf("    Found match: %s (ID: %s)\n", games$name[i], games$id[i]))
        return(games$id[i])
      }

      # Try partial matches for common team names (e.g., "Arsenal" in "Arsenal FC")
      home_parts <- strsplit(tolower(home_team), "\\s+")[[1]]
      away_parts <- strsplit(tolower(away_team), "\\s+")[[1]]

      home_match <- any(sapply(home_parts, function(p) nchar(p) > 3 && grepl(clean_name(p), game_clean)))
      away_match <- any(sapply(away_parts, function(p) nchar(p) > 3 && grepl(clean_name(p), game_clean)))
...
get_espn_mlb_player_stats <- function(game_id, player_name) {
  ...
  tryCatch({
    ...
    for (team_idx in 1:2) {
      ...
      # Try both batting and pitching stats
      for (stat_type in c("batting", "pitching")) {
        stat_idx <- which(team_stats$type == stat_type)
            ...
            if (grepl(player_name, current_name, ignore.case = TRUE)) {
              ...
              if (length(raw_stats) == length(stat_labels)) {
                ...
                if (stat_type == "batting") {
                  ...
                  # Pitching stats
                  ip_str <- as.character(raw_stats["IP"])
                  ip_val <- suppressWarnings(as.numeric(ip_str))

                  # Calculate outs from IP (e.g., 5.1 -> 16 outs)
                  ip_parts <- strsplit(as.character(ip_str), "\\.")[[1]]
                  full_innings <- as.numeric(ip_parts[1]) %||% 0
                  partial_outs <- if(length(ip_parts) > 1) as.numeric(ip_parts[2]) %||% 0 else 0
                  outs_recorded <- (full_innings * 3) + partial_outs
                  ...
                  result <- list(
                    ...
                    hits_allowed = suppressWarnings(as.numeric(raw_stats["H"])),
                    runs_allowed = suppressWarnings(as.numeric(raw_stats["R"])),
                    earned_runs = suppressWarnings(as.numeric(raw_stats["ER"])),
                    walks_allowed = suppressWarnings(as.numeric(raw_stats["BB"])),
                    pitching_strikeouts = suppressWarnings(as.numeric(raw_stats["K"])),
                    home_runs_allowed = suppressWarnings(as.numeric(raw_stats["HR"])),
                    pitches_thrown = suppressWarnings(as.numeric(strsplit(as.character(raw_stats["PC-ST"]), "-")[[1]][1])),
                    era = suppressWarnings(as.numeric(raw_stats["ERA"]))
                  )
                }
    ...
    debug_cat(sprintf("  Player '%s' not found in MLB stats for game %s\n", player_name, game_id))
    return(NULL)
...
get_espn_nba_player_stats <- function(game_id, player_name, league = "nba") {
  tryCatch({
    url <- paste0("https://site.api.espn.com/apis/site/v2/sports/basketball/", league, "/summary?event=", game_id)
    debug_cat(sprintf("  Calling ESPN API for %s player stats: %s\n", league, url))
...
find_game_id <- function(home_team, away_team, game_date, sport, team_only_mode = FALSE) {
  ...
  # Backup check for NCAA games - DISABLED
  if (FALSE && (sport == "nfl" || sport == "football")) {
...
# ==============================================
# MLB PLAYER STATS WRAPPER (matches fetch_nba/nfl pattern)
# ==============================================
...
fetch_mlb_player_stats <- function(player_name, season, game_date = NULL, game_id = NULL) {
  ...
  tryCatch({
    if (!is.null(game_id) && !is.na(game_id) && game_id != "NA" && game_id != "unknown") {
      debug_cat("  Using ESPN API for MLB player stats...\n")
      player_stats <- get_espn_mlb_player_stats(game_id, player_name)

      if (!is.null(player_stats)) {
        debug_cat(sprintf("  Found player stats via ESPN API\n"))
        debug_cat(sprintf("    Hits: %s, Runs: %s, RBI: %s, HR: %s, SB: %s\n",
                          player_stats$hits, player_stats$runs, player_stats$rbi,
                          player_stats$home_runs, player_stats$stolen_bases))
        ...
        return(list(
          ...
          stats = list(
            # Hitting stats
            at_bats = player_stats$at_bats %||% 0,
            runs = player_stats$runs %||% 0,
            hits = player_stats$hits %||% 0,
            rbi = player_stats$rbi %||% 0,
            walks = player_stats$walks %||% 0,
            strikeouts = player_stats$strikeouts %||% 0,
            home_runs = player_stats$home_runs %||% 0,
            doubles = player_stats$doubles %||% 0,
            triples = player_stats$triples %||% 0,
            stolen_bases = player_stats$stolen_bases %||% 0,
            singles = (player_stats$hits %||% 0) - ((player_stats$doubles %||% 0) + (player_stats$triples %||% 0) + (player_stats$home_runs %||% 0)),
            total_bases = (player_stats$hits %||% 0) + (player_stats$doubles %||% 0) + 2 * (player_stats$triples %||% 0) + 3 * (player_stats$home_runs %||% 0),

            # Pitching stats
            pitching_strikeouts = player_stats$pitching_strikeouts %||% 0,
            innings_pitched = player_stats$innings_pitched %||% 0,
            outs_recorded = player_stats$outs_recorded %||% 0,
            hits_allowed = player_stats$hits_allowed %||% 0,
            runs_allowed = player_stats$runs_allowed %||% 0,
            earned_runs = player_stats$earned_runs %||% 0,
            walks_allowed = player_stats$walks_allowed %||% 0,
            home_runs_allowed = player_stats$home_runs_allowed %||% 0
          )
        ))
      } else {
        debug_cat("  ESPN API returned NULL player stats\n")
    ...
    debug_cat("ERROR: No game_id provided for ESPN MLB API lookup\n")
    return(list(found = FALSE, error = "No game_id available for ESPN MLB API"))
...
get_espn_league_slug <- function(league_name) {
  ...
  if (grepl("serie a|italy", league_name)) return("ita.1")
  if (grepl("ligue 1|france", league_name)) return("fra.1")
  if (grepl("champions league|uefa champions", league_name)) return("uefa.champions")
  if (grepl("europa league|uefa europa", league_name)) return("uefa.europa")
  if (grepl("mls|major league soccer|usa mls", league_name)) return("usa.1")
  if (grepl("eredivisie|netherlands|holland", league_name)) return("ned.1")
  if (grepl("primeira liga|portugal", league_name)) return("por.1")
  if (grepl("liga mx|mexico", league_name)) return("mex.1")
  if (grepl("brasileiro|brazil", league_name)) return("bra.1")
  if (grepl("argentina|primera div", league_name) && !grepl("spain", league_name)) return("arg.1")
  if (grepl("belgian|belgium|pro league", league_name)) return("bel.1")
  if (grepl("scottish|scotland|premiership", league_name)) return("sco.1")
  if (grepl("turkish|turkey|super lig", league_name)) return("tur.1")
  if (grepl("conference league", league_name)) return("uefa.conference")
  if (grepl("fa cup", league_name)) return("eng.fa")
  if (grepl("efl cup|carabao cup", league_name)) return("eng.league_cup")
  if (grepl("copa del rey", league_name)) return("esp.copa_del_rey")
  if (grepl("dfb pokal", league_name)) return("ger.pokal")
  if (grepl("coppa italia", league_name)) return("ita.coppa_italia")
...
get_espn_soccer_player_stats <- function(game_id, league_slug, player_name) {
  url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/soccer/%s/summary?event=%s", league_slug, game_id)
  debug_cat(sprintf("  Fetching soccer stats from: %s\n", url))

  res <- tryCatch({
    jsonlite::fromJSON(url)
  }, error = function(e) {
    debug_cat(sprintf("  Error fetching from ESPN: %s\n", e$message))
    return(NULL)
  })

  if (is.null(res) || is.null(res$rosters)) {
    debug_cat("  No rosters found in ESPN response.\n")
    return(NULL)
  }

  # Search for player in rosters
  found_stats <- NULL

  # jsonlite simplifies the rosters list into a data frame
  rosters_df <- res$rosters
  if (is.data.frame(rosters_df)) {
    for (team_idx in 1:nrow(rosters_df)) {
      team_roster <- rosters_df$roster[[team_idx]]
      if (is.null(team_roster)) next

      # team_roster is a data frame where 'athlete' is a data frame column
      # and 'stats' is a list of data frames
      for (p_idx in 1:nrow(team_roster)) {
        display_name <- team_roster$athlete$displayName[p_idx]
        if (grepl(tolower(player_name), tolower(display_name))) {
          found_stats <- team_roster$stats[[p_idx]]
          break
        }
      }
      if (!is.null(found_stats)) break
    }
  } else if (is.list(rosters_df)) {
    # Fallback if it's still a list
    for (team_idx in 1:length(rosters_df)) {
      team_roster <- rosters_df[[team_idx]]$roster
      if (is.null(team_roster)) next
  ...
  if (is.null(found_stats)) {
    debug_cat(sprintf("  Player '%s' not found in rosters.\n", player_name))
    return(NULL)
  }
  ...
  # found_stats is a data frame with columns: name, displayName, shortDisplayName, description, abbreviation, value, displayValue
  if (!is.null(found_stats) && is.data.frame(found_stats)) {
    for (i in 1:nrow(found_stats)) {
      name <- found_stats$name[i]
      val <- as.numeric(found_stats$value[i])

      if (name == "totalGoals") stats_list$goals <- val
      if (name == "goalAssists") stats_list$assists <- val
      if (name == "totalShots") stats_list$shots <- val
      if (name == "shotsOnTarget") stats_list$shots_on_target <- val
      if (name == "yellowCards") stats_list$cards_yellow <- val
      if (name == "redCards") stats_list$cards_red <- val
      if (name == "foulsCommitted") stats_list$fouls_committed <- val
      if (name == "foulsSuffered") stats_list$fouls_drawn <- val
    }
  }
...
fetch_soccer_player_stats <- function(player_name, league, game_id = NULL) {
  debug_cat(sprintf("\nDEBUG fetch_soccer_player_stats:\n"))
  debug_cat(sprintf("  Player: %s\n", player_name))
...
fetch_game_result <- function(game_id, sport, season) {
  ...
  tryCatch({
    if (sport %in% c("nba", "basketball")) {
      ...
      # Add WNBA support
      debug_cat("  Using ESPN Summary API for WNBA...\n")
      summary_url <- sprintf("https://site.api.espn.com/apis/site/v2/sports/basketball/wnba/summary?event=%s", game_id)
      ...
      if (!is.null(summary_response) && is.list(summary_response)) {
        if (!is.null(summary_response$header) && !is.null(summary_response$header$competitions)) {
          competitions <- summary_response$header$competitions
          ...
          if (is.data.frame(competitions) && "competitors" %in% names(competitions) &&
            ...
            if (is.data.frame(competitors_df) && nrow(competitors_df) >= 2) {
              ...
              if (nrow(home_rows) > 0 && nrow(away_rows) > 0) {
                ...
                debug_cat(sprintf("  WNBA game result: %s %d - %d %s\n",
                                  result$away_team, result$away_score,
                                  result$home_score, result$home_team))
                debug_cat(sprintf("  Total points: %d\n", result$total_points))
              }
            }
          }
        }
...
resolve_multi_inning_total_runs <- function(game_data, num_innings, line_value, direction, team = NULL) {
      ...
      if ("competitors" %in% names(comp) && length(comp$competitors) > 0) {
        ...
        if (is.data.frame(competitors) && nrow(competitors) >= 2) {
          ...
          if (!is.null(team) && nchar(team) > 0) {
              ...
              if (grepl(target_team, competitors$team.displayName[i], ignore.case = TRUE)) {
                if ("linescores" %in% names(competitors) && length(competitors$linescores) >= i) {
                  ...
                  if (!is.null(linescores_df) && is.data.frame(linescores_df)) {
                    inning_runs <- as.numeric(linescores_df$displayValue)
                    # Remove NA values (in case of extra innings)
                    inning_runs <- inning_runs[!is.na(inning_runs)]
                    total_runs <- sum(inning_runs[1:min(num_innings, length(inning_runs))], na.rm = TRUE)
                    debug_cat(sprintf("  Team %s inning runs: %s\n", team, paste(inning_runs, collapse=", ")))
                    debug_cat(sprintf("  Team %s runs in first %d innings: %d\n", team, num_innings, total_runs))
                    break
                  }
                }
              }
            ...
            # ========== Original behavior: sum both teams (game total) ==========
            debug_cat(sprintf("  Calculating game total (both teams) for first %d innings\n", num_innings))
  ...
  tryCatch({
    ...
    if (sport == "wnba" && market_info$type == "game_total") {
      ...
      if (game_data$found) {
        ...
        debug_cat(sprintf("  Final score: %s %d - %d %s\n",
                          ...
                          game_data$home_score, game_data$home_team))
        debug_cat(sprintf("  Total points: %d\n", game_data$total_points))
        
        if (!is.null(line_value) && !is.null(direction)) {
          line_num <- as.numeric(line_value)
          if (direction == "over") {
            result$bet_won <- game_data$total_points > line_num
          } else if (direction == "under") {
            result$bet_won <- game_data$total_points < line_num
          }
          result$line_value <- line_num
          debug_cat(sprintf("  Line: %.1f %s -> Bet won: %s\n",
                            line_num, direction, result$bet_won))
        }
      } else {
    ...
    if (sport == "mlb" && grepl("total.*strikeouts", market_lower)) {
      ...
      if (combo_result$success) {
        ...
        if (!is.null(line_value)) {
          bet_res <- check_bet_won(combo_result$actual_value, line_value, market_lower, direction = direction)
          if (!is.null(bet_res$won)) {
            result$bet_won <- bet_res$won
            result$line_value <- bet_res$line
      ...
      if (special_result$success) {
        ...
        if (market_info$special_type %in% c("double_double", "triple_double")) {
          ...
          if (!is.null(line_value) && !is.null(special_result$actual_value)) {
            bet_res <- check_bet_won(special_result$actual_value, line_value, market_lower, direction = direction)
            if (!is.null(bet_res$won)) {
              result$bet_won <- bet_res$won
              result$line_value <- bet_res$line
      ...
      if (market_info$stat == "total_bases") {
        if (sport %in% c("mlb", "baseball")) {
          ...
          if (!is.null(stats_data) && stats_data$found) {
            ...
            if (!is.null(actual_value) && !is.na(actual_value)) {
              result$actual_value <- actual_value
              if (!is.null(line_value)) {
                bet_res <- check_bet_won(actual_value, line_value, market_lower, direction = direction)
                if (!is.null(bet_res$won)) {
                  result$bet_won <- bet_res$won
                  result$line_value <- bet_res$line
                }
        ...
        if (sport %in% c("nhl", "hockey")) {
          ...
          if (!is.null(stats_data)) {
            ...
            if (!is.null(line_value)) {
              bet_res <- check_bet_won(actual_value, line_value, market_lower, player_name, direction)
              if (!is.null(bet_res$won)) {
                result$bet_won <- bet_res$won
                result$line_value <- bet_res$line
              }
          ...
          debug_cat("  Fetching Soccer player stats...\n")
          league <- result$league %||% "English Premier League"
          stats_data <- fetch_soccer_player_stats(player_name, league, game_id = game_id)
      ...
      if (market_info$type %in% c("period_moneyline", "period_moneyline_3way")) {
        ...
        # Check for swapped match type from find_game_id
        game_info_match_type <- if (!is.null(result$game_info$match_type)) result$game_info$match_type else "exact"
        
        result_data <- resolve_period_spread(game_id, sport, market_info$period, line_value, bet_team, match_type = game_info_match_type)
        result$success <- TRUE
        result$resolved <- TRUE
        result$data <- result_data
        if (!is.null(result_data$bet_won)) result$bet_won <- result_data$bet_won
        ...
        period_result <- resolve_period_total(game_id, sport, result$season, market_info$period, line_value, direction)
        result$success <- TRUE
        result$resolved <- TRUE
      ...
      if (market_subtype == "total_runs") {
        ...
        # Determine bet team for ML markets
        bet_team_arg <- if(!is.null(teams$team_only) && teams$team_only) "home" else player_name
        result_data <- resolve_multi_inning_market(game_id, market_info$innings, market_subtype,
                                                   line_value, direction, bet_team_arg)
      }
      ...
      game_data <- fetch_game_data_for_resolution(game_id, sport)
      if(!is.null(game_data)) {
        result_data <- resolve_multi_inning_total_runs(game_data, num_innings, line_value, direction, team = bet_team)
        result$success <- result_data$success
        result$resolved <- result_data$success
        result$data <- result_data
        if(!is.null(result_data$bet_won)) result$bet_won <- result_data$bet_won
      ...
      if(!is.null(game_data)) {
        ...
        result$data <- result_data
        if(!is.null(line_value)) {
          bet_res <- check_bet_won(result_data$actual_value, line_value, market_lower, direction = direction)
          if (!is.null(bet_res$won)) {
            result$bet_won <- bet_res$won
            result$line_value <- bet_res$line
          }
    ...
    } else if (sport == "wnba" && market_info$type == "game_total") {
      debug_cat("Market type: WNBA game total points\n")
      ...
      if (market_info$type == "player_period_stat") {
        ...
        if (period_data$success) {
          ...
          if (market_info$type == "game_moneyline") {
            ...
            if (!is.null(line_value)) {
              bet_res <- check_bet_won(total_goals, line_value, market_lower, direction = direction)
              if (!is.null(bet_res$won)) {
                result$bet_won <- bet_res$won
                result$line_value <- bet_res$line
              }
        ...
        if (period_data$success) {
          ...
          if (market_info$type == "regulation_moneyline") {
            ...
            bet_res <- check_bet_won(result$regulation_total, line_value, market_lower, direction = direction)
            if (!is.null(bet_res$won)) {
              result$bet_won <- bet_res$won
              result$line_value <- bet_res$line
            }
      ...
      if (period_result$success) {
        ...
        result$data <- period_result
        if (!is.null(line_value)) {
          bet_res <- check_bet_won(period_result$total, line_value, market_lower, direction = direction)
          if (!is.null(bet_res$won)) {
            result$bet_won <- bet_res$won
            result$line_value <- bet_res$line
      ...
      if (game_data$found) {
        ...
        result$resolved <- TRUE
        result$data <- game_data
        bet_res <- check_bet_won(game_data$total_points, line_value, market_lower, direction = direction)
        if (!is.null(bet_res$won)) {
          result$bet_won <- bet_res$won
          result$line_value <- bet_res$line
          result$actual_value <- game_data$total_points
...
extract_cumulative_scores <- function(game_id, sport, innings = NULL) {
  debug_cat(sprintf("  Extracting inning %s scores\n", ifelse(is.null(innings), "all", innings)))
  
  library(jsonlite)
  
  # Fetch the game data directly from the summary endpoint
  summary_url <- paste0("https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/summary?event=", game_id)
  summary_data <- fromJSON(summary_url, flatten = TRUE)
  
  if (is.null(summary_data$header$competitions)) {
    return(list(home_score = 0, away_score = 0, total = 0, error = "Could not get game data"))
  }
  
  # Extract competitions data
  comp <- summary_data$header$competitions
  competitors <- comp$competitors[[1]]
  
  home_score <- 0
  away_score <- 0
  
  # Process each team's linescores
  for (i in 1:nrow(competitors)) {
    if (!is.null(competitors$linescores[[i]])) {
      linescores_df <- competitors$linescores[[i]]
      
      # CRITICAL FIX: Convert displayValue to numeric (was character)
      inning_runs <- as.numeric(linescores_df$displayValue)
      
      # Remove NA values (in case of extra innings)
      inning_runs <- inning_runs[!is.na(inning_runs)]
      
      if (competitors$homeAway[i] == "home") {
        if (!is.null(innings)) {
          home_score <- sum(inning_runs[1:min(innings, length(inning_runs))], na.rm = TRUE)
        } else {
          home_score <- sum(inning_runs, na.rm = TRUE)
        }
        debug_cat(sprintf("    Home team (%s) inning runs: %s\n", 
                          competitors$team.displayName[i], 
                          paste(inning_runs, collapse=", ")))
      } else if (competitors$homeAway[i] == "away") {
        if (!is.null(innings)) {
          away_score <- sum(inning_runs[1:min(innings, length(inning_runs))], na.rm = TRUE)
        } else {
          away_score <- sum(inning_runs, na.rm = TRUE)
        }
        debug_cat(sprintf("    Away team (%s) inning runs: %s\n", 
                          competitors$team.displayName[i], 
                          paste(inning_runs, collapse=", ")))
      }
    }
  ...
  debug_cat(sprintf("    Cumulative: Home=%d, Away=%d -> Total=%d\n", home_score, away_score, total_score))
...
# Placeholder for removed duplicate function
# ==============================================
# PERIOD SPREAD RESOLVER
# ==============================================

resolve_period_spread <- function(game_id, sport, period, line_value, bet_team, match_type = "exact") {
  debug_cat(sprintf("  Resolving %s spread: %s @ %s (match_type: %s)\n", period, bet_team, line_value, match_type))
  period_result <- resolve_period_moneyline(game_id, sport, period)
  if (match_type == "swapped") {
    debug_cat("  ⚠️ ADJUSTING SPREAD: Teams were reversed in API response\n")
    point_diff <- period_result$away_score - period_result$home_score
  } else {
    point_diff <- period_result$home_score - period_result$away_score
  }
  line_num <- as.numeric(line_value)
  result <- list(point_diff = point_diff, line_value = line_num, home_score = period_result$home_score, away_score = period_result$away_score, match_type = match_type)
  if (bet_team == "home") { result$bet_won <- (point_diff + line_num) > 0 }
  else if (bet_team == "away") { result$bet_won <- (-point_diff + line_num) > 0 }
  else { result$bet_won <- FALSE }
  debug_cat(sprintf("  Point diff: %+d, Line: %s, Bet team: %s -> Won: %s\n", point_diff, line_value, bet_team, result$bet_won))
  return(result)
}
...
resolve_multi_inning_market <- function(game_id, num_innings, market_type, line_value = NULL, direction = NULL, bet_team = NULL) {
  ...
  # Ensure cumulative scores are valid numbers
  home_score <- if(!is.null(cumulative$home_score) && length(cumulative$home_score) > 0) as.numeric(cumulative$home_score) else 0
  away_score <- if(!is.null(cumulative$away_score) && length(cumulative$away_score) > 0) as.numeric(cumulative$away_score) else 0
  total_score <- if(!is.null(cumulative$total) && length(cumulative$total) > 0) as.numeric(cumulative$total) else (home_score + away_score)

  result$home_score <- home_score
  result$away_score <- away_score
  result$total <- total_score
  result$innings <- num_innings
  result$success <- TRUE # Data fetched successfully

  if (market_type %in% c("moneyline", "moneyline_3way")) {
    ...
    debug_cat(sprintf("    Scores: Home=%d, Away=%d -> Winner: %s\n", home_score, away_score, result$winner))

    if (!is.null(bet_team) && length(bet_team) > 0) {
      debug_cat(sprintf("    Bet Team: '%s'\n", bet_team))
      if (bet_team %in% c("Draw", "tie", "Tie", "Draw/Tie")) {
        result$bet_won <- (result$winner == "tie")
      } else {
        # Use safe comparisons to avoid 'length 0' errors
        is_home_bet <- identical(bet_team, "home")
        is_away_bet <- identical(bet_team, "away")

        if (is_home_bet) result$bet_won <- (result$winner == "home")
        else if (is_away_bet) result$bet_won <- (result$winner == "away")
        else {
          result$bet_won <- (result$winner == bet_team)
        }
      }
    }
    if (!is.null(result$bet_won)) debug_cat(sprintf("    Bet Won: %s\n", result$bet_won))
  } else if (market_type == "total_runs") {
    line_num <- suppressWarnings(as.numeric(line_value))
    if (!is.na(line_num)) {
      result$bet_won <- if (direction == "over") total_score > line_num else total_score < line_num
      result$actual_value <- total_score
      result$line_value <- line_num
    }
  } else if (market_type == "both_teams_score") {
    result$both_scored <- (home_score > 0 && away_score > 0)
    result$bet_won <- result$both_scored
  }
